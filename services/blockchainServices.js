...
const userWalletEntry = async (user_id, address, hash, created) => {
  ...
  const src = await generateQR(address);
    ...
    if (type == "ETH") {
      ...
      if (blockNumber) {
        await Tokendetails.update(
          { _id: id },
          { $set: { payment_status: "paid", block_id: blockNumber } }
        );
      }
    } else {
...
const importWalletFindId = async (id) => {
  let importwallet = await Importwallet.findOne({
    user_id: id,
    login_status: "login",
  });
  console.log("iddd", id);
...
const findTransactions = async (address) => {
  let transactions = await Tokendetails.find({
    $or: [
