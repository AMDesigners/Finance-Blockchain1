...
const transactionManagement = async function (req, res) {
  ...
  try {
    ...
    for (let i = 0; i < transactionData.length; i++) {
      let userDet = await Registration.findOne({
        _id: transactionData[i].userId,
      });
      let user_name = "";
      if (userDet) {
      ...
      if (
        transactionData[i].operation == "Send" ||
        transactionData[i].operation == "Sell"
      ) {
        userTransInfo.debit =
          transactionData[i].amount + transactionData[i].coinname;
...
const dashboardPage = async (req, res) => {
  ...
  if (test != true) {
    ...
    if (loginwallet) {
      let result = await blockchainServices.userWalletFindId(
        loginwallet.wallet_id
      );
      console.log("in dashboard results", result);
      if (result) {
        req.session.src = result.src;
        req.session.wallet = true;
        let wallet_creation = result.created_at;
        let today = await userServices.createAtTimer();
        let wallet_time_difference = calculateHours(
          new Date(wallet_creation),
          new Date(today)
        );
        wallet_details = result;
        import_wallet_id = loginwallet._id;
        let all_transaction = await blockchainServices.findTransactions(
          wallet_details.wallet_address
        );
        await blockchainServices.checkTxStatus(all_transaction);
        all_transaction = await blockchainServices.findTransactions(
          wallet_details.wallet_address
        );
        let balance = await blockchainServices.getCoinBalance(
          wallet_details.wallet_address
        );
...
const submitUser = async (req, res) => {
  ...
  if (user) {
    ...
    if (req.body.password == req.body.conf_pass) {
      ...
      let user = await userServices.checkUser(req.body.email);
      if (ref_link != "") {
        let refData = await userServices.referData(
          user.ref_code,
          ref_link,
          user._id,
          created
      ...
      let otp = user.otp;
      let subject =
        "OTP for your new account on The Finance Blockchain website";
      let text =
        "Hello " +
        req.body.email +
        ",<br><br>\n\nCongratulations on signing up with The Finance Blockchain website!<br><br>\n\n" +
        "Your one-time password (OTP) for signing up is: <strong>" +
        otp +
        "</strong>. This would be valid only for the next 10 minutes." +
        // '<br><br>\n\nOnce you enter the OTP and create a new wallet, we will credit it by 10 $FBT (worth US$10)  as a limited-time joining bonus.<br><br>\n\n' +
        // 'Moreover, you can earn more by referring your friends and earn US$5 equivalent $FBT tokens every time your friend joins by using your referral code. Your friend will also get US$5 equivalent $FBT tokens for using your referral code !<br><br>\n\n' +
        "Time: " +
        created +
        "<br><br>\n\n";
      "If this withdrawal attempt was not made by you it means someone visited your account. It may be an indication you have been the target of a phishing attempt and might want to consider moving your funds to a new wallet." +
        "<br><br>\n\n" +
        "Regards,<br>\nTeam The Finance Blockchain<br>\nhttps://ebtico.com";
      try {
...
const walletSuccess = async (req, res) => {
  ...
  if (test != true) {
    ...
    if (req.query.wallet) {
      wallet_address = Buffer.from(req.query.wallet, "base64").toString(
        "ascii"
      );
    }
...
//                     req.session.re_usr_email = user.email;
//                     req.session.is_user_logged_in = false;
//                     let subject = 'OTP for your new account on Finance Blockchain website';
//                     let text = 'Hello ' + req.body.email + ',<br><br>\n\nCongratulations on signing up with Finance Blockchain website!<br><br>\n\n' +
//                         'Your one-time password (OTP) for signing up is: ' + otp + '. This would be valid only for the next 10 minutes.' +
//                         '<br><br>\n\nOnce you enter the OTP and create a new wallet, we will credit it by 50 Finance Blockchain (worth US$50)  as a limited-time joining bonus.<br><br>\n\n' +
//                         'Moreover, you can earn more by referring your friends and earn US$10 equivalent Finance Blockchain tokens every time your friend joins by using your referral code. Your friend will also get US$10 equivalent Finance Blockchain tokens for using your referral code !<br><br>\n\n' +
//                         'Time: ' + user.created + '<br><br>\n\n'
//                     'If this withdrawal attempt was not made by you it means someone visited your account. It may be an indication you have been the target of a phishing attempt and might want to consider moving your funds to a new wallet.' + '<br><br>\n\n' + 'Regards,<br>\nFinance Blockchain Team<br>\nhttps://ebtico.com';
//                     await mail(req.body.email, subject, text);
//                     req.flash('success_msg', 'User registered. Please verify to continue.');
//                     res.redirect('/verify-account');
//                 }
...
const submitForgot = async (req, res) => {
  ...
  if (!user) {
    ...
    let userUpdated = await userServices.updateUserPassword(
      req.body.email,
      mystr1
    );
    if (userUpdated) {
      ...
      let text =
        ...
        "If this password change attempt was not made by you it means someone visited your account. It may be an indication you have been the target of a phishing attempt and might want to consider moving your funds to a new wallet." +
        "<br><br>\n\n" +
        "Regards,<br>\nFinance Blockchain Team<br>\nhttps://ebtico.com";
      await mail(req.body.email, subject, text);
      req.flash(
        "success_msg",
        "Password has been sent successfully to your registered email."
      );
      res.redirect("/forgot-pass");
    } else {
      req.flash("err_msg", "Something went wrong.");
...
const gettxdate = async (req, res) => {
  let sender = req.body.sender;
  let txs = await blockchainServices.findTransactionsDate(
    sender,
    req.body.date
  );
  res.send({ txs });
