...
router.get("/receive-fbt", isUser, function (req, res) {
  ...
  if (test != true) {
    ...
    Userwallet.findOne({ _id: walletid }, function (err, response) {
      if (err) {
        console.log("Something is worng to find login status.");
      } else {
  ...
  if (test != true) {
    ...
    Registration.findOne({ _id: user_id }, function (err, result) {
      if (err) {
        ...
        // res.send(result);
        req.session.re_usr_name = result.name;
        res.render("profile", {
          err_msg,
          success_msg,
          result,
...
router.post("/update-profile", isUser, async function (req, res) {
  ...
  if (status == 1) {
    Registration.updateOne(
      { _id: user_id },
      { $set: { name: name, email: email, mobile_no: mob } },
      { upsert: true },
      function (err, result) {
        if (err) {
          console.log("Something went wrong");
      ...
      function (err, loginwallet) {
        if (err) {
          ...
          if (
            loginwallet != "" &&
            loginwallet != null &&
            loginwallet != undefined
          ) {
            Userwallet.findOne(
              { _id: loginwallet.wallet_id },
              function (err, addresponse) {
                if (err) {
                  console.log("Something is worng to Token details.");
                } else {
                  var user_wallet = addresponse.wallet_address;

                  Tokendetails.find({
                    $or: [
                      { receiver_wallet_address: addresponse.wallet_address },
                      { sender_wallet_address: addresponse.wallet_address },
                    ],
                  })
                    .sort([["auto", -1]])
                    .exec(function (err, response) {
                      if (err) {
                        ...
                        res.render("transaction-table", {
                          err_msg,
                          success_msg,
                          user_wallet,
                          all_transaction,
                          address: addresponse.wallet_address,
                          layout: false,
                          session: req.session,
                          moment,
                        });
